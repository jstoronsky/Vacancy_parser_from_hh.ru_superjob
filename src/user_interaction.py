...
def user_interaction():
    ...
    if input_platform not in ['hh', 'HH', 'headhunter', 'HeadHunter', 'sj', 'SJ', 'Superjob', 'SuperJob']:
        ...
        if input_platform in ['hh', 'HH', 'headhunter', 'HeadHunter']:  # блок кода, если платформа HH
            ...
            if len(hh_sorted_area) == 0:
                ...
                for work_in_city in hh_sorted_area:
                    print(work_in_city)
        elif input_platform in ['sj', 'SJ', 'Superjob', 'SuperJob']:  # блок кода, если платформа SuperJob
            sj_vacancies = SuperJobApi(input_page_number, input_keyword).get_info_from_site()
            JSon('vacancies_from_sj.json', sj_vacancies).write_info()
